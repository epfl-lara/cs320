...
### 3.2 Write less to do more (3)

Add support for implicit parameters and conversions to Amy, alongside more integer sizes to take advantage of them.

Implicit parameters should behave as follows, similarly to Scala:
```scala
...
Implict conversions allow specified functions to act as implicit conversions, just like in Scala:

```scala
implicit def i2b(i: Int): Boolean := !(i == 0) end i2b
...
Finally, adding more integer sizes (e.g. `Int(8)`, `Int(16)`, and/or `Int(64)`) will allow you to demonstrate the usefulness of implicit conversions, e.g. by allowing implicit conversions from smaller to larger integer types. You can start by adding `Int(64)`. Those should support the same operations as `Int(32)` (`+`, `*`, ...), and you should allow implicit conversions between them:
...
You will have to think if you want to support narrowing conversions and how:
...
Tuples must be immutable, and elements in a tuple can be of different types.
...
```scala
operator 50 def +(i1: Int, i2: Int): Int := error("+") end +
```
...
```scala
def foo(i: Int, j: Int = 42): Int := i + j end foo
...
def foo(i: Int = 5, j: Int): Int := i + j end foo
...
```scala
inline def abs(n: Int): Int := if (n < 0) then -n else n end if end abs
...
```scala
def foo(n: Int): Int :=
  def plus1(n: Int): Int :=  n + 1 end plus1
  inline def times2(n: Int): Int :=  2 * n  end times2
...
Allow more of Scala 3 syntaxes in Amy. Amy syntax currently mixes braces-style (e.g. `match`) and end markers-style (e.g. `if`, `object`).
...
def countFrom(start: Int): List := Cons(start, countFrom(start + 1)) end countFrom
